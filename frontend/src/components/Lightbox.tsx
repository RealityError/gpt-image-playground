...
export default function Lightbox() {
  ...
  const editCurrentImageMask = useCallback(() => {
    if (!lightboxImageId) return
    setLightboxImageId(null)
    setMaskEditorImageId(lightboxImageId)
  }, [lightboxImageId, setLightboxImageId, setMaskEditorImageId])
  ...
  return (
    <LightboxInner
      ...
      onPrev={goPrev}
      onNext={goNext}
      onMaskEdit={editCurrentImageMask}
    />
  )
}
...
function LightboxInner({ src, imageId, maskPreviewSrc, onClose, showNav, currentIndex, total, onPrev, onNext, onMaskEdit }: LightboxInnerProps) {
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className="absolute right-4 top-4 z-20 inline-flex items-center gap-1.5 rounded-full bg-black/50 px-3 py-1.5 text-xs font-medium text-white backdrop-blur-sm transition hover:bg-black/70"
        title="局部重绘"
      >
