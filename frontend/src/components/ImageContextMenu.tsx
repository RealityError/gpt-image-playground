...
export default function ImageContextMenu() {
  ...
  const getServerImageUrl = (imageId?: string) => {
    if (!imageId) return ''
    for (const task of useStore.getState().tasks) {
      const url = task.serverImageUrls?.[imageId]
      if (url) return url
    }
    return ''
  }
  ...
  const getOriginalImageSource = async () => {
    const serverUrl = getServerImageUrl(menuInfo.imageId)
    if (!menuInfo.imageId) return { src: menuInfo.src, fallbackUrl: serverUrl || menuInfo.src }
    const cachedOrServerUrl = await ensureImageCached(menuInfo.imageId)
    return {
      src: cachedOrServerUrl || serverUrl || menuInfo.src,
      fallbackUrl: serverUrl || menuInfo.src,
    }
  }
  ...
  const handleCopy = async (e: React.MouseEvent) => {
    ...
    setMenuInfo(null)
    try {
      const { src, fallbackUrl } = await getOriginalImageSource()
  ...
  const handleDownload = async (e: React.MouseEvent) => {
    ...
    setMenuInfo(null)
    try {
      const { src } = await getOriginalImageSource()
