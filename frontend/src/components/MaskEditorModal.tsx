...
import { MAX_MASK_EDIT_FILE_BYTES } from '../lib/imageApiShared'
import { DEFAULT_MASK_WORKING_MAX_EDGE, prepareMaskTargetDataUrl, replaceMaskTargetImage } from '../lib/maskPreprocess'
          ...
          <div className="relative flex items-center gap-1.5">
            ...
            {showMaskInfo && (
              <div className="absolute left-0 top-full mt-2 w-64 rounded-xl border border-gray-200/80 bg-white px-3 py-2 text-xs leading-5 text-gray-600 shadow-lg dark:border-white/[0.08] dark:bg-gray-900 dark:text-gray-300">
                ...
                画透明区域作为重绘范围。超过 {DEFAULT_MASK_WORKING_MAX_EDGE}px 边长的图片会先缩小；提交时遮罩主图和遮罩文件单个不能超过 {formatMiB(MAX_MASK_EDIT_FILE_BYTES)}。
              </div>
            )}
          </div>
        </div>
